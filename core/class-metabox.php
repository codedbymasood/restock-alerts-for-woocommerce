	...
	private function render_repeater_field( $field, $post, $field_value ) {
		...
		if ( ! empty( $repeater_values ) && is_array( $repeater_values ) ) {
			foreach ( $repeater_values as $index => $repeater_value ) {
				...
				echo '<span class="repeater-title">' . esc_html__( 'Item: ', 'restock-alerts-for-woocommerce' ) . ' ' . esc_html( ( (int) $index + 1 ) ) . '</span>';
		...
		echo '<div class="repeater-header">';
		echo '<span class="repeater-title">' . esc_html__( 'Item: ', 'restock-alerts-for-woocommerce' ) . ' {INDEX_DISPLAY}</span>';
		echo '<span class="remove-item"><span class="dashicons dashicons-no-alt"></span></span>';
		echo '</div>';
		echo '<div class="repeater-content">';
		...
		echo '</div>';
		echo '<button type="button" class="button add-repeater-item">' . esc_html__( 'Add Item', 'restock-alerts-for-woocommerce' ) . '</button>';
	}
		...
		switch ( $field['type'] ) {
			...
			case 'media':
				...
				printf(
					...
					esc_html__( 'Select Media', 'restock-alerts-for-woocommerce' )
				);

				echo '<button type="button" class="button remove-media-button">' . esc_html__( 'Remove All', 'restock-alerts-for-woocommerce' ) . '</button>';
