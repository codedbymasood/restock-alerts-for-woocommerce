...
<div class="stobokit-wrapper">
	<div class="nav-tab-wrapper horizontal">
		<div class="nav-tabs">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=stobokit-status&tab=status' ) ); ?>" class="nav-tab<?php echo 'status' === $current_tab ? ' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Status', 'restock-alerts-for-woocommerce' ); ?></a>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=stobokit-status&tab=logs' ) ); ?>" class="nav-tab<?php echo 'logs' === $current_tab ? ' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Logs', 'restock-alerts-for-woocommerce' ); ?></a>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=stobokit-status&tab=>email-logs' ) ); ?>" class="nav-tab<?php echo 'email-logs' === $current_tab ? ' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Email Logs', 'restock-alerts-for-woocommerce' ); ?></a>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=stobokit-status&tab=scheduled-actions' ) ); ?>" class="nav-tab<?php echo 'scheduled-actions' === $current_tab ? ' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Scheduled Actions', 'restock-alerts-for-woocommerce' ); ?></a
		</div>
	</div>
		...
		<div class="nav-tab-content <?php echo 'status' === $current_tab ? 'active' : ''; ?>">
			<div class="stobokit-wrapper no-spacing">
				<div class="status-table wrap">
					<h1><?php esc_html_e( 'Plugin Status', 'restock-alerts-for-woocommerce' ); ?></h1>
					<?php
					$status_data = array(
			...
			<div class="stobokit-wrapper no-spacing">
				<?php if ( empty( $logs ) ) : ?>
					<div class="log-container">
						<div class="log-entry">
							<p><?php esc_html_e( 'No debug logs have been recorded yet.', 'restock-alerts-for-woocommerce' ); ?></p>
						</div>
					</div>
				<?php else : ?>
					<div class="log-container">
				...
				<div class="form-group">
					<form method="post">
						...
						<button type="submit" class="button button-primary"><?php esc_html_e( 'Download Logs', 'restock-alerts-for-woocommerce' ); ?></button>
					</form>

					<form method="post">
						<?php wp_nonce_field( 'clear_logs' ); ?>
						<input type="hidden" name="action" value="clear_logs">
						<button type="submit" class="button button-primary"><?php esc_html_e( 'Clear Logs', 'restock-alerts-for-woocommerce' ); ?></button>
					</form>
				</div>
			</div>
		</div>
		...
		<div class="nav-tab-content <?php echo 'email-logs' === $current_tab ? 'active' : ''; ?>">
			<div class="status-table wrap">
				...
				if ( 'email-logs' === $current_tab ) {
					$args = array(
						'title'      => esc_html__( 'Email Logs', 'restock-alerts-for-woocommerce' ),
						'singular'   => 'email_log',
						'plural'     => 'email_logs',
