...
class License {
	...
	public function activate_license() {
		if ( ! isset( $_POST['nonce'] ) || empty( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'stobokit_license' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Sorry, not verified.', 'restock-alerts-for-woocommerce' ),
				)
			);
		}
				...
				switch ( $data->error ) {
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'restock-alerts-for-woocommerce' ),
							date_i18n( get_option( 'date_format' ), strtotime( $data->expires, current_time( 'timestamp' ) ) )
						);
						break;
					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'restock-alerts-for-woocommerce' );
						break;
					case 'missing':
						$message = esc_html__( 'Invalid license.', 'restock-alerts-for-woocommerce' );
						break;
					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'restock-alerts-for-woocommerce' );
						break;
					case 'invalid_item_id':
						/* translators: 1: Plugin name */
						$message = esc_html__( 'This appears to be an invalid license key for this product.', 'restock-alerts-for-woocommerce' );
						break;
					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'restock-alerts-for-woocommerce' );
						break;
					default:
						$message = esc_html__( 'An error occurred, please try again.', 'restock-alerts-for-woocommerce' );
						break;
				}
			}
		}
		...
		if ( ! empty( $message ) ) {
			...
			wp_send_json_success(
				array(
					'message'     => esc_html__( 'License activated succesfully!', 'restock-alerts-for-woocommerce' ),
					'status'      => 'active',
	...
	public function deactivate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( isset( $license_data->license ) && in_array( $license_data->license, array( 'deactivated', 'failed' ), true ) ) {
				...
				wp_send_json_success(
					array(
						'message' => esc_html__( 'License deactivated succesfully!', 'restock-alerts-for-woocommerce' ),
						'status'  => 'inactive',
					)
				);
			}
