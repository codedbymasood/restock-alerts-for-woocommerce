...
class Onboarding {
	...
	public function __construct( $args = array() ) {
		$defaults = array(
			...
			'textdomain'    => '',
			'steps'         => array(),
			'page_slug'     => 'restock-alerts-for-woocommerce',
			'option_prefix' => 'restock-alerts-for-woocommerce',
		);
	...
	public function register_onboarding_page() {
		add_submenu_page(
			'stobokit',
			esc_html__( 'Onboarding', 'restock-alerts-for-woocommerce' ),
			esc_html__( 'Onboarding', 'restock-alerts-for-woocommerce' ),
			'manage_woocommerce',
			$this->page_slug,
		...
		<div class="stobokit-step-content">
			...
			if ( $this->step_file_exists( $this->step ) ) {
				...
				<div class="notice notice-error">
					<p><?php esc_html_e( 'Step file missing. Please contact support.', 'restock-alerts-for-woocommerce' ); ?></p>
				</div>
				<?php
			}
			?>
		...
		<div class="stobokit-step-actions">
			<?php if ( $current_pos > 0 ) : ?>
				...
				<a class="btn stobokit-button--prev" href="<?php echo esc_url( add_query_arg( 'step', $prev_step ) ); ?>">
					&larr; <?php esc_html_e( 'Back', 'restock-alerts-for-woocommerce' ); ?>
				</a>
			<?php endif; ?>
			...
			<?php if ( $current_pos < count( $step_keys ) - 1 ) : ?>
				...
				<a class="btn stobokit-button--next" href="<?php echo esc_url( add_query_arg( 'step', $next_step ) ); ?>">
					<?php esc_html_e( 'Continue', 'restock-alerts-for-woocommerce' ); ?> &rarr;
				</a>
			<?php else : ?>
				<a class="btn stobokit-button--finish" href="<?php echo esc_url( $this->redirect_page . '&onboarding=complete' ); ?>">
					<?php esc_html_e( 'Finish', 'restock-alerts-for-woocommerce' ); ?>
				</a>
			<?php endif; ?>
		</div>
		<?php
	...
	public function render_page() {
		...
		<div class="stobokit-wrapper stobokit-onboarding">
			<h1><?php esc_html_e( 'Getting Things Ready', 'restock-alerts-for-woocommerce' ); ?></h1>
