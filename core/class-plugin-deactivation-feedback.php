...
class Plugin_Deactivation_Feedback {
	...
	public function deactivation_feedback_form() {
		...
		<div class="hidden stobokit-wrapper" id="plugin-deactivation-modal">
			...
			<div class="plugin-deactivation-content">
					<h2><?php esc_html_e( 'Quick Feedback', 'restock-alerts-for-woocommerce' ); ?></h2>
					<p><?php esc_html_e( 'If you have a moment, please share why you\'re deactivating the plugin:', 'restock-alerts-for-woocommerce' ); ?></p>
					...
					<form id="plugin-deactivation-form">
						<ul class="deactivation-reasons">
							<li>
								<label>
									...
									<span><?php esc_html_e( 'It\'s a temporary deactivation', 'restock-alerts-for-woocommerce' ); ?></span>
								</label>
							</li>
							<li>
								<label>
									<input type="radio" name="reason" value="stopped_working" />
									<span><?php esc_html_e( 'The plugin stopped working', 'restock-alerts-for-woocommerce' ); ?></span>
								</label>
							</li>
							<li>
								<label>
									<input type="radio" name="reason" value="found_better_plugin" />
									<span><?php esc_html_e( 'I found a better plugin', 'restock-alerts-for-woocommerce' ); ?></span>
								</label>
							...
							<li>
								<label>
									...
									<span><?php esc_html_e( 'The plugin lacks a feature I need', 'restock-alerts-for-woocommerce' ); ?></span>
								</label>
							...
							<li>
								<label>
									...
									<span><?php esc_html_e( 'The plugin is difficult to use', 'restock-alerts-for-woocommerce' ); ?></span>
								</label>
							</li>
							<li>
								<label>
									<input type="radio" name="reason" value="other" />
									<span><?php esc_html_e( 'Other reason', 'restock-alerts-for-woocommerce' ); ?></span>
								</label>
						...
						<div class="deactivation-actions">
							<button type="submit" class="button button-primary"><?php esc_html_e( 'Submit & Deactivate', 'restock-alerts-for-woocommerce' ); ?></button>
							<button type="button" class="button button-tertiary" id="skip-deactivation"><?php esc_html_e( 'Skip & Deactivate', 'restock-alerts-for-woocommerce' ); ?></button>
						</div>
					</form>
			</div>
		</div>
	...
	public function handle_feedback() {
		...
		if ( $hubspot_sent ) {
			wp_send_json_success(
				array(
					'message' => esc_html__( 'Feedback sent successfully', 'restock-alerts-for-woocommerce' ),
				)
			);
		} else {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Failed to send', 'restock-alerts-for-woocommerce' ),
				)
			);
		}
	...
	private function send_to_hubspot( $data ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'HubSpot API Error', 'restock-alerts-for-woocommerce' ),
				)
			);
			return false;
		}
